...
folder links, the coach's Google account) is normally never committed.** It lives in a single
...
**Exception:** `curriculum/classroom/Welcome_Email.md` **is** tracked, despite having real
coach names and live Drive folder links, by the coach's explicit request after being told the
specific risk (public repo, live folder-sharing links, hard to fully retract once pushed).
Don't "fix" this by re-gitignoring it without being asked again.
