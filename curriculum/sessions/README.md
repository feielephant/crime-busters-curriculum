# Crime Busters — Practice Session Scripts (Year 2)

12 full coach scripts, 90 minutes each, plus 3 practice-test sessions run in
between. Run in order — every session assumes the tools and vocabulary from
earlier ones.

**Read each script once before the practice.** It is written so that, having
skimmed it ahead of time, you can run the session straight from the page:
timed segments, exact talking points in `COACH SCRIPT:` blocks, `[ASK THE TEAM]`
prompts, `[CHECKPOINT]` gut-checks, `[ACTIVITY]` lab stations sized for a team of
2 or 4, a wrap-up, and a "common kid mistakes" list at the end. Each script also
has an **"If you only have 60 minutes"** cut path near the top.

Pairs with `../Crime_Busters_Knowledge_Document.md` — keep it open during practice
for questions that go past the script.

## This is a second-year curriculum

The team is assumed to have had a full first season on the basics. Each session
opens with a 5–10 minute **prerequisite recap** of the year-1 material for that
topic, then spends the rest of the time on what real invitational tests actually
demand and year 1 never covered. If your team is new to the event, you will need
